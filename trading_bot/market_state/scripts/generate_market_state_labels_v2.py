...
def classify_market_state(df):
    ...
    bear_mask = (
        ...
        (df['RSI'] < 45) & 
        (df['MACD_Histogram'] < 0) & 
        (df['OBV'] < df['OBV'].rolling(window=30).mean())  
    )
    
    ranging_mask = (
        (df['ADX'].rolling(window=10).max() < 20) &  # ADX < 20 for 10+ days
        (df['close'].rolling(window=10).apply(lambda x: (x.max() - x.min()) / x.mean()) < 0.05) &  # Price within ±5% of 200-MA for 10+ days
        (df['ATR'] < df['ATR'].rolling(window=50).mean())  
    )
...
# Main function
def main():
    input_file_path = '/Users/jameserskine/Documents/trading_bot/datasets/market_state_labeled copy.csv'
    output_file_path = '/Users/jameserskine/Documents/trading_bot/datasets/market_state_labeled_v2.csv'

    df = load_historical_data(input_file_path)
    df = compute_indicators(df)
    df = classify_market_state(df)

    df.to_csv(output_file_path, index=False)

    print(f"Dataset saved to {output_file_path}. Review classifications manually.")
