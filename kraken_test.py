...
# Use all available USDT/USD pairs
...
def backtest(symbol):
    ...
    # Log the raw data to understand what we are working with
    print(f"Fetched data for {symbol}: {ohlcv}")
    ...
    # Log the difference between latest price and historical high
    print(f"Latest Price: {latest_price}, Historical High: {historical_high}")
    print(f"Difference: {latest_price - historical_high}")

    breakout_margin = historical_high * 1.03  # Relaxed margin (3% above the highest price)

    if latest_price > breakout_margin:
